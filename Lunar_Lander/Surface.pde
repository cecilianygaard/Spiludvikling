      ...
      for (PVector point : corners) {
        ...
        if (p1.x <= point.x && p2.x >= point.x) {
          if (pointUnderLine(point, p1, p2)) {
            ...
            } else if (s.velocity.y <= 20 && (s.angle%(2*PI) < (2*PI-1) && s.angle%(2*PI-1) > 1) || (s.angle%(2*PI) < -1 && s.angle%(2*PI) > (-2*PI+1))) {//If it has hit the platform, but the angle or the y-velocity wasn't "correct"
              s.alive = false;
              shipDestroyed = new ShipFragments(s.location);
